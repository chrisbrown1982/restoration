#line 228 "/Users/chrisb/Documents/Papers/hlpp2020/splash2/codes/null_macros/c.m4.null"
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the x,
...

  {
	...
	//for (i = 0; i < (P) - 1; i++) {
		Error = 0;
        // pthread_create(&PThreadTable[i], NULL, (void * (*)(void *))(SlaveStart), NULL);
        SlaveStart();
		if (Error != 0) {
...
};
  {
	...
	for (i = 0; i < (P) - 1; i++) {
		Error = 0;
        // pthread_join(PThreadTable[i], NULL);
		if (Error != 0) {
...
/*
  ...
  /* transpose from x into scratch */
    for (int i = 0 ; i < P ; i++) {
        ...
        MyFirst = rootN*MyNum/P;
        MyLast = rootN*(MyNum+1)/P;
  Transpose(n1, x, scratch, i, MyFirst, MyLast, pad_length);
...
/*
  ...
  /* transpose */
    for (int i = 0 ; i < P ; i++) {
        ...
        MyFirst = rootN*MyNum/P;
        MyLast = rootN*(MyNum+1)/P;
  Transpose(n1, scratch, x, i, MyFirst, MyLast, pad_length);
