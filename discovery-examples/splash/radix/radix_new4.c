...
#line 230
}*/
   {
	...
	Error = 0; // pthread_mutex_init(&(global->barrier_rank).mutex, NULL);
	if (Error != 0) {
		printf("Error while initializing barrier.\n");
		exit(-1);
	}

	Error = 0; pthread_cond_init(&(global->barrier_rank).cv, NULL);
	if (Error != 0) {
		printf("Error while initializing barrier.\n");
...
}
   {
	...
	Error = 0; // pthread_mutex_init(&(global->barrier_key).mutex, NULL);
	if (Error != 0) {
		printf("Error while initializing barrier.\n");
		exit(-1);
	}

	Error = 0; // pthread_cond_init(&(global->barrier_key).cv, NULL);
	if (Error != 0) {
		printf("Error while initializing barrier.\n");
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the key,
   ...
   
   {
	...
	for (i = 0; i < (number_of_processors) - 1; i++) {
		Error = 0;// pthread_create(&PThreadTable[i], NULL, (void * (*)(void *))(slave_sort), NULL);
        slave_sort();
...
};
   {
	unsigned long	i, Error;
	for (i = 0; i < (number_of_processors) - 1; i++) {
		Error = 0; // pthread_join(PThreadTable[i], NULL);
		if (Error != 0) {
